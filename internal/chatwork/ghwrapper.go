...
// installGHWrapperV2 writes a `gh` shim to a fresh dir and returns that dir for
// prepending to the bash tool's PATH — same PATH-prepend contract as
// installGHWrapper, but the shim delegates the per-call fetch to selfPath's
// hidden "gh-wrapper" subcommand instead of shell+grep, since provisioned mode
// needs an HTTP GET + JSON decode a POSIX shell cannot do without assuming
// curl/jq are present in the image.
...
// injected token. This matches v1's wrapper, which always execs gh
// best-effort, and keeps repo-less gh calls (gh --version, gh auth status,
// gh api /user, gh repo list, gh repo create) working even when a credential
// can't be minted.
